...
import { Badge, buttonClass, Card, Field, inputClass, PageHeader, secondaryButtonClass, textareaClass } from "@/components/ui";
...
export default async function EditSourcePage({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      ...
      {placeholderReason ? (
        <Card className="mb-4 border-rose-200 bg-rose-50/70 p-4 text-sm leading-6 text-rose-800">
          <div className="mb-1"><Badge tone="danger">プレースホルダー</Badge></div>
      ...
      <Card className="p-4">
        <form action={updateWatchSource} className="grid gap-4 md:grid-cols-4">
          ...
          <Field label="ソース名"><input className={inputClass} name="name" defaultValue={source.name} required /></Field>
          <Field label="店舗名"><input className={inputClass} name="storeName" defaultValue={source.storeName} required /></Field>
          <Field label="URL"><input className={inputClass} name="url" type="url" defaultValue={source.url} required /></Field>
          <Field label="種別">
          ...
          <div className="md:col-span-3">
            <Field label="メモ"><textarea className={textareaClass} name="memo" defaultValue={source.memo ?? ""} /></Field>
          </div>
          <label className="flex items-end gap-2 pb-2 text-sm font-medium text-muted-foreground">
            <input name="enabled" type="checkbox" defaultChecked={source.enabled && !placeholderReason} disabled={Boolean(placeholderReason)} />
            有効
          </label>
